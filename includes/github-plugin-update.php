...
function pw_get_merged_pw_settings() {
	...
	$stored = get_option( 'pw_settings', array() );
	if ( ! is_array( $stored ) ) {
		$stored = array();
	}
 ...
 * @return mixed
 */
function pw_get_setting( $key, $default = null ) {
	$settings = pw_get_merged_pw_settings();
 ...
 * @param string|null         $zip_basename  Asset filename to match (default: plugin zip).
 * @return array{zip_url:string,tag_name:string}|null
 */
function pw_github_zip_from_release_payload( $data, $zip_basename = null ) {
	if ( ! is_array( $data ) ) {
		return null;
	}
	$tag    = isset( $data['tag_name'] ) ? (string) $data['tag_name'] : '';
	$assets = isset( $data['assets'] ) && is_array( $data['assets'] ) ? $data['assets'] : array();
	$name   = is_string( $zip_basename ) && $zip_basename !== '' ? $zip_basename : PW_GITHUB_PLUGIN_RELEASE_ZIP;
 ...
 *
 * @param string|null $zip_basename Release asset filename to find (default: plugin zip constant).
 * @return array{release:array<string,mixed>,package:array{zip_url:string,tag_name:string}|null}|WP_Error
 */
function pw_github_resolve_plugin_release( $releases_url, $zip_basename = null ) {
	...
	$asset_label = is_string( $zip_basename ) && $zip_basename !== '' ? $zip_basename : PW_GITHUB_PLUGIN_RELEASE_ZIP;
	$latest_api = sprintf(
	...
	if ( $code === 200 ) {
		...
		return array(
			'release' => $data,
			'package' => pw_github_zip_from_release_payload( $data, $zip_basename ),
		);
	}
		...
		if ( $list_code === 200 ) {
			...
			if ( is_array( $list ) ) {
				foreach ( $list as $release ) {
					$pkg = pw_github_zip_from_release_payload( $release, $zip_basename );
					if ( $pkg !== null ) {
						return array(
							'release' => $release,
							'package' => $pkg,
...
function pw_github_get_latest_release_package( $releases_url ) {
	return pw_github_get_latest_release_zip_by_asset( $releases_url, PW_GITHUB_PLUGIN_RELEASE_ZIP );
}
...
function pw_github_get_latest_release_zip_by_asset( $releases_url, $zip_basename ) {
	$zip_basename = is_string( $zip_basename ) ? trim( $zip_basename ) : '';
	if ( $zip_basename === '' ) {
		return new WP_Error( 'pw_github_bad_asset', __( 'Missing release asset filename.', 'portico-webworks' ) );
	}
	$r = pw_github_resolve_plugin_release( $releases_url, $zip_basename );
	if ( is_wp_error( $r ) ) {
		return $r;
	}
	if ( $r['package'] === null ) {
...
function pw_render_github_update_tab() {
	...
	if ( ! current_user_can( 'update_plugins' ) ) {
		...
		echo '<p class="description">' . esc_html__( 'You need permission to update plugins to use one-click updates from GitHub.', 'portico-webworks' );
		echo ' <a href="' . esc_url( $plugins_url ) . '">' . esc_html__( 'Open Plugins', 'portico-webworks' ) . '</a>.</p>';
		echo '</div></div>';
		return;
	}
